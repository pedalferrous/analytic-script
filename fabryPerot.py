...
def main():
    ...
    t0 = time.clock()
    # representing CQD layer as active
    activeLayerIndex = 1
    for dim0 in range(len(searchSpace)):
        for dim1 in range(len(searchSpace[0])):
            for dim2 in range(len(searchSpace[0][0])):
                ...
                # initialize values in given cell
                initialParameters = [[init0, init1, init2]]
                ...
                searchSpace[dim0][dim1][dim2] = (str(init0) + ":" + str(init1) + ":" + str(init2) , ESqIntAvg[activeLayerIndex])
    t1 = time.clock()
