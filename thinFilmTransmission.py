...
# variable indicates layer properties stemming from 2d lattice
film = namedtuple('layer',['depth','index','name','active','variable'])
...
def genMatrices(stack, wls, angles, n_i, n_f, indices, t_angles):
    ...
    I = [[[[
        IMatrix(n_i, indices[i][j], angles[k], t_angles[i][j][k], l, False, wls[j], i) if i == 0
        else IMatrix(indices[i-1][j], n_f, t_angles[i-1][j][k], t_angles[i][j][k], l, False, wls[j], i) if i == len(stack)
        else IMatrix(indices[i-1][j], indices[i][j], t_angles[i-1][j][k], t_angles[i][j][k], l, stack[i-1].variable, wls[j], i-1)
        for l in [0,1]] 
        for k in range(len(angles))] 
...
def IMatrix(n_1, n_2, theta_1, theta_2, pol, isVariable, wavelength, layerIndex):
    # standard 1d homogenous layer
    if not isVariable:   
    ...
    # layer with unique resonance characteristics
    # determine how this should be altered for differing polarizations
    # for variable layers, given possible absoprtion into the material,
    # one needs to account for both bulk properties separately
    else:
        if (pol==0): # perpendicular (s-) polarization
            t = getTransmission(wavelength, layerIndex)
            r = t - 1.0
        elif (pol==1): # parallel (p-) polarization
            t = getTransmission(wavelength, layerIndex)
            r = t - 1.0
